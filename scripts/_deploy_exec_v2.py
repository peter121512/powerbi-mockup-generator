"""Deploy Executive Overview page using the template system (v2).

Proves the Stage 07e page can be fully expressed through the declarative
template/binding system. Uses DesignTokens + TemplateRegistry + PageBuilder
to generate the PBIR definition from executive_config, then deploys via
Fabric REST API and captures a headless screenshot.
"""
import sys
import json
import base64
...
visual_archives: dict[str, tuple[bytes, bytes]] = {
    ...
    GAUGE_GUID: load_visual_archive(GAUGE_GUID, "premiumGauge"),
}
...
html_path = evidence_dir / "_exec_v2.html"
...
with sync_playwright() as p:
    ...
    page.screenshot(path=str(evidence_dir / "exec_overview_v2.png"))
    browser.close()
html_path.unlink(missing_ok=True)
print("Done! Check exec_overview_v2.png")
